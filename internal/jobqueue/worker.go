...
// Handler performs one idempotent unit of work.
type Handler func(context.Context, Job) Outcome

type queueBackend interface {
	Acquire(context.Context, string, string, time.Duration) (Job, bool, error)
	Start(context.Context, string, string) error
	Heartbeat(context.Context, string, string, time.Duration) error
	Succeed(context.Context, string, string) error
	Fail(context.Context, string, string, Failure, time.Time) (State, error)
	CancelActive(context.Context, string, string) error
}
...
func NewWorker(queue *Queue, config WorkerConfig, handlers map[string]Handler) (*Worker, error) {
	if queue == nil || !validName(config.Queue) || strings.TrimSpace(config.Owner) == "" ||
		len(config.Owner) > 255 ||
		...
		config.RetryMaximum < config.RetryMinimum {
	...
	return &Worker{queue: queue, config: config, handlers: copied}, nil
}
...
func (worker *Worker) runSlot(ctx context.Context, owner string) error {
	for {
		job, ok, err := worker.queue.Acquire(
			ctx,
			worker.config.Queue,
			owner,
...
func (worker *Worker) process(ctx context.Context, job Job) error {
	if err := worker.queue.Start(ctx, job.ID, job.LeaseToken); err != nil {
		if errors.Is(err, ErrCancellationRequested) {
			return worker.queue.CancelActive(ctx, job.ID, job.LeaseToken)
		}
		return err
	}
	handler := worker.handlers[job.Kind]
	if handler == nil {
		_, err := worker.queue.Fail(ctx, job.ID, job.LeaseToken, Failure{
	...
	case OutcomeSucceeded:
		err := worker.queue.Succeed(ctx, job.ID, job.LeaseToken)
		if errors.Is(err, ErrCancellationRequested) {
			return worker.queue.CancelActive(ctx, job.ID, job.LeaseToken)
		}
		return err
	...
	case OutcomeRetryable:
		if !validCode(outcome.Code) {
			return worker.failInvalidOutcome(ctx, job)
		}
		delay := outcome.RetryAfter
		if delay <= 0 {
			delay = jitteredRetryDelay(
				job.AttemptCount,
				worker.config.RetryMinimum,
				worker.config.RetryMaximum,
			)
		}
...
func (worker *Worker) failInvalidOutcome(ctx context.Context, job Job) error {
	_, err := worker.queue.Fail(ctx, job.ID, job.LeaseToken, Failure{
		Class: ErrorPermanent,
